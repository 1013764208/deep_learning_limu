...
def self_attention(query, key, value, dropout=None, mask=None):
    ...
    # key.transpose(-2, -1)) 将key张量的最后两个维度进行转置，因为在内积运算中，要求'key'的最后一个维度要与'query'的倒数第二个维度相匹配，故转置
    ...
    return torch.matmul(self_attn_softmax, value), self_attn_softmax  # QKV
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, head, d_model, dropout=0.1):
        """
        :param head: 头数
        :param d_model: 词向量的维度，必须是head的整数倍 （模型的内部表示维度或隐藏层的维度大小）
        :param dropout: drop比率
        """
        ...
        其中linear_out是针对多头汇总时给予的参数 W_Q W_K W_V
        """
