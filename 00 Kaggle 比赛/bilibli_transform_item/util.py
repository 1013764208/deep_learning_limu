...
def attention(Q, K, V, mask):
    ...
    # Q,K矩阵相乘，求每个词相对其他词的注意力
    score = torch.matmul(Q, K.permute(0, 1, 3, 2))

    # 除以每个头维数的平方根，做数值缩放
    score /= 8 ** 0.5

    # mask掩盖，mask是true的地方都被替换-inf，这样在计算softmax的时候，-inf会被压缩到0
    # mask = [b, 1, 50, 50]
    score = score.masked_fill_(mask, -float('inf'))
    score = torch.softmax(score, dim=-1)  # 最后一维
...
class MultiHead(torch.Module):
    def __init__(self):
        ...
        self.norm = torch.nn.LayerNorm(
            normalized_shape=32, elementwise_affine=True)

        self.dropput = torch.nn.Dropout(p=0.1)
    ...
    def forward(self, Q, K, V, mask):
        ...
        # 拆分成多个头
        # b句话，每句话50词，每个词编码成32维向量，4个头，每个头分到8维向量
        # [b, 50 ,32] -> [b, 4, 50, 8]
        ...
        # 计算输出，维度不变
        # [b, 50, 32] -> [b, 50, 32]
        score = self.dropput(self.out_fc(score))
    ...
    # 位置编码层
    class PostitionEmbedding(torch.nn.Module):
        def __init__(self):
            super().__init__()

            # pos是第几个词，i是第几个维度，d_modle是维度总数
